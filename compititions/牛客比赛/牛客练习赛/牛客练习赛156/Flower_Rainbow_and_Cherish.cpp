...
void solve(){
  ...
  constexpr int M = 31;
  std::vector<std::array<i64, 2>> cnt(M), sum(M);
  std::vector<i128> diff(M), sam(M);

  auto new_node = [&](int u, i64 x){
    for(int k = 0; k < M; k++){
      int t = (a[u] >> k) & 1;
      cnt[k][t] += x;
      sum[k][t] += x * dep[u];
  ...
  std::function<void(int)> dfs = [&](int u) {
    ...
    for(int k = 0; k < M; k++){
      ...
      i64 c0 = cnt[k][0], c1 = cnt[k][1];
      i64 s0 = sum[k][0], s1 = sum[k][1];

      i64 d2 = (t == 0 ? c1 * d - s1 : c0 * d - s0);
      i64 s = (t == 0 ? c0 * d - s0 : c1 * d - s1);
  ...
  for(int k = 0; k < M; k++){
    ans += (static_cast<i128>(1) << k) * std::min(diff[k], sam[k]);
  }

  std::cout << Z(static_cast<int>(ans % P)) << "\n";
}
